...
2.  Import the CSV file titled 'country_level_data_0.csv', contained in the 'data' directory with help of the `read_csv()` function
...
___ <- read_csv("wk-01/data/country_level_data_0.csv")
```
...
5.  Describe in your own words what the `mutate()` function does on the following two lines
...
6.  Run the code contained in the code-chunk and fix any errors
7.  Look at the plot that you created and once again describe what the `mutate()` function achieved in the previous task
8.  Describe in your own words what the 'color' argument inside the `aes()` achieves?
9.  Next to the code-chunk option `#| eval:` change the value from `false` to `true`
10. Render and fix any errors
...
1.  In the YAML header, replace `html` with `pdf`
...
2.  Navigate to the generated PDF: `wk-01/hw-01-data-science-lifecycle.pdf`
...
![](img/export-file-from-rscloud.png)
