...
1.  Use the `read_csv()` function to import the 'country_level_data_0.csv' file from the wk-03/data folder by assigning it to an object name `global_waste_data`
...
global_waste_data <- read_csv(here::here("wk-03/data/country_level_data_0.csv"))
```
...
```{r}
composition_tidy_capita_income_cat <- read_rds(here::here("wk-03/data/01-msw-generation-and-composition-by-income.rds"))
```
...
table_composition <- read_rds(here::here("wk-03/data/03-table-composition.rds"))
table_msw_generation <-  read_rds(here::here("wk-03/data/02-table-msw-generation.rds"))
...
4.  Navigate to the generated PDF: `wk-03/hw-03a-data-transformation.pdf`
