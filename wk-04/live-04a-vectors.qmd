...
tib_data %>% 
  mutate(vector_num = as.numeric(date)) %>% 
  mutate(date2 = lubridate::as_date("1960-05-19")) %>% 
  mutate(date2_num = as.numeric(date2)) %>% 
  mutate(month = month(date2, label = TRUE),
         weekday = wday(date2, label = TRUE, abbr = FALSE)) %>% 
  mutate(factor = factor(vector_chr, levels = c("small", "medium", "large"))) %>% 
  mutate(factor_num = as.numeric(factor))

as.numeric("1960-05-19")
typeof(1960-05-19)

lubridate::as_date("1960-05-19")

as.numeric(factor(vector_chr, levels = c("small", "medium", "large")))
...
waste_data <- read_delim(here::here("wk-04/data/rbtl_fs22_waste_composition_bellevue.csv"), delim = ";")
