...
1.  Use the `read_csv()` (**Note**: Watch out for the `_` don't use the `.` as in `read.csv()`) function to import the 'msw-generation-and-composition-by-income.csv' and assign it to an object with the name `waste_data`. **Tipp:** Combine the `read_csv()` with the `here()` function as we have done in the live coding exercise of week 3.

```{r}

waste_data <- read_csv(here("wk-04/data/msw-generation-and-composition-by-income.csv"))
...
1.  Starting with the `waste_data` object, use the pipe operator and the `mutate()` function to convert the `income_cat` variable from a variable of type character to a variable of type factor. Define the factor levels yourself and store them in a separate object with a name of your choice.

**Tipp:** First create a frequency table using the following code to identify the correct spelling of the categories in the variable `income_cat`.
...
1.  Starting with the `waste_data` object, use the `pivot_longer()` function to convert the dataframe from a wide to a long format. Apply the following:
...
waste_data_long <- read_rds(here("wk-04/data/msw-generation-and-composition-by-income-long.rds"))
...
waste_data_long_mean <- read_rds(here("wk-04/data/msw-generation-and-composition-by-income-long-mean.rds"))
...
4.  Navigate to the generated PDF: `wk-04/hw-04a-tidyr.pdf`
